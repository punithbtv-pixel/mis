...
function makeReading(id, date, prev = null) {
  ...
  return {
    ...
    utilityDgHours: (prev?.utilityDgHours ?? 380) + 3.1,
    adminDgHours: (prev?.adminDgHours ?? 82) + 1.2,
...
export function upsertMockReading(body) {
  ...
  if ("dieselIssuances" in body) {
    next.dieselIssuances = Array.isArray(body.dieselIssuances)
      ? body.dieselIssuances.map((it) => {
      ...
      : [];
  } else if (!("dieselIssuances" in next)) {
    next.dieselIssuances = [];
  }
